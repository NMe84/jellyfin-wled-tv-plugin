...
- **Letterbox / pillarbox handling** — optional, and toggled independently for horizontal (letterbox) and vertical (pillarbox) bars. When a video's aspect ratio differs from the screen (e.g. a 4:3 video on a 16:9 TV), the LEDs over the resulting black bars are switched off and the rest map to where the picture actually is, instead of stretching it across the whole strip. Bars baked into the video file are also handled, using symmetric detection (a bar must appear on both opposing sides) capped at 21:9 for letterboxing and 4:3 for pillarboxing so genuinely dark content is not mistaken for a bar. Resampled every 2 seconds, so dynamic aspect-ratio changes (e.g. IMAX sequences) are handled automatically
