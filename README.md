...
| Capture method | **Canvas 2D** (default) works on most platforms. Switch to **WebGL** on devices where the video decoder renders to a hardware overlay inaccessible to Canvas 2D, such as LG WebOS. On both methods the video is nudged out of the hardware-overlay plane (a near-invisible `brightness(1.001)` filter) so its frames stay readable — this is what keeps sampling working on WebOS 26 |
